...
ListaPessoa* RemovePessoa(ListaPessoa* h, int nc, ListElem **historico) {
	...
	{
		//procura pessoa a remover
		ListaPessoa* aux = h;
		ListaPessoa* auxAnt = aux;
		while (aux) {
			if (aux->fichaPessoa.nc == nc) break;	//se encontrou
			auxAnt = aux;
			aux = aux->proxPessoa;
		}
		//Remover pessoa
		...
		auxAnt->proxPessoa = aux->proxPessoa;
		//Registar no Historico
		aux->proxPessoa = NULL;
		*historico = addItemHead(*historico, aux);
	}
	return h;
}
